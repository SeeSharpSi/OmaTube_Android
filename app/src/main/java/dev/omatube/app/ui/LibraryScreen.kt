...
/**
 * Library entry point shared by the full and simple UIs. Renders the feed,
 * history, or Watch Next route, the touch navigation row (replacing the
...
fun LibraryScreen(
    ...
    BoxWithConstraints(
        ...
        if (error != null) {
            Box(modifier = Modifier.align(Alignment.BottomCenter).padding(18.dp)) {
                OmaErrorBanner(
                    message = error,
                    panel = colors.lighterBackground,
                    danger = colors.red,
