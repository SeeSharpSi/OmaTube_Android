...
Full UI renders edge to edge: the Activity root paints `LocalOmaColors.current.background` behind the status and gesture navigation bars while non-player content still respects `safeDrawingPadding`, and system-bar icon appearance follows the background luminance. On older releases that still honor opaque resource bars, the non-player Full UI also themes the status, navigation and navigation-divider window colors to the palette background, while Simple UI and the player restore the window theme colors. Phone-width feed content and the accent dividers run to the safe-area edges while ordinary chrome (live row, category bar, bottom navigation) keeps a 20 dp container inset and the Watch Next/History grids retain their existing internal padding; Simple UI keeps its prior 56 dp side margin. Full UI shows refresh status and error banners as transient top-right notices that auto-hide four seconds after work is inactive and dismiss on tap, while Simple UI keeps its persisted bottom-anchored notices. Watch Next and Watch History drop their top headers in Full UI only; the Watch Next count moves beside the bottom-bar title in a weighted group that keeps the five 38 dp navigation controls fixed. The full refresh button swaps its glyph for the existing braille spinner while refreshing at the same 38 dp size.
