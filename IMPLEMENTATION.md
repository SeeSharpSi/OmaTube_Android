...
- NewPipeDownloader bootstraps NewPipeExtractor v0.26.5's YouTube service-worker client version from `sw.js_data`, retaining the original `sw.js` request as a compatibility fallback. Successful service-worker-data parsing avoids the extractor's fragile HTML fallback.
...
Seek-bar contact suppresses landscape chrome auto-hide while held, and releasing it restarts the full idle delay. The scrub timestamp is centered above the bottom controls rather than following the handle. Portrait layout is keyed on physical orientation: the display-cutout-safe container pins the 52 dp top slot and aspect-fitted Media3 surface to the top, places the transcript directly below the video, anchors the bottom control slot above a 24 dp screen-edge gap, and reserves at least 180 dp for transcript text. Top and bottom chrome remain visible; the center transport alone toggles on video tap and hides after three seconds while playing.

The black timestamp-and-cue list maps measured text offsets to word timestamps for seeking. Its persistent timestamp-gutter input and marker share the list's padded viewport. During a scrub, the marker moves in physical pixels at 1.75 times finger travel, independent of the timestamp-to-cue mapping used for highlights and seek previews. It remains visible across row gaps and lazy item disposal. When the marker reaches either edge, it stays 2 dp inside the viewport and a frame loop scrolls the list at the amplified pointer velocity. Holding retains the last outward velocity; reversal moves the marker inward and stops edge scrolling. A crossing's residual distance is consumed once, without adding that frame's velocity step. Release commits one seek, while cancellation commits nothing. A paused committed preview keeps its physical marker until playback advances, avoiding timestamp snapping on release. Normal playback follow rapidly catches up at its separate 10,000 dp-per-second rate and stays disabled during scrubbing.

Playback position drives active-cue scrolling, progressive word highlighting, and the playback marker. `PlayerController` loads JSON3 caption data asynchronously from the already-resolved `StreamInfo`, so caption failure or latency never blocks playback. Landscape keeps full-bleed video, tap-to-toggle chrome, and the three-second idle timeout. Loading renders without landscape chrome. The fullscreen icon derives its state from the current orientation rather than a stored flag.
